...
export default function AdminDocuments() {
  ...
  const handleFileSelect = (file: File) => {
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      toast({
        title: tc("error"),
        description: t("fileSizeError", { max: MAX_FILE_SIZE_MB }),
        variant: "destructive",
      })
      return
    }
  ...
  const uploadFileToServer = async (file: File): Promise<string | null> => {
    ...
    try {
      ...
      toast({ title: t("uploadError"), description: message, variant: "destructive" })
      return null
    }
  }
  ...
  const uploadDocument = async () => {
    if (!uploadForm.title.trim()) {
      toast({ title: tc("error"), description: t("titleRequired"), variant: "destructive" })
      return
    }

    if (uploadMode === "file" && !selectedFile && !uploadedUrl) {
      toast({ title: tc("error"), description: t("selectFileRequired"), variant: "destructive" })
      return
    }

    if (uploadMode === "url" && !uploadForm.fileUrl.trim()) {
      toast({ title: tc("error"), description: t("urlRequired"), variant: "destructive" })
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: tc("success"), description: t("documentAdded") })
  ...
  const deleteDocument = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: tc("deleted"), description: t("documentDeleted") })
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {t("downloadCount", { count: doc.downloadCount })}
                      </span>
                      {doc.uploadedBy && <span>{t("uploadedBy", { name: doc.uploadedBy.fullName })}</span>}
                    </div>
                  </div>
      ...
      <Dialog open={uploadDialogOpen} onOpenChange={(open) => { setUploadDialogOpen(open); if (!open) resetUploadDialog() }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-5 py-2">
                  ...
                  >
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t("maxFileSize", { max: MAX_FILE_SIZE_MB })}
                      </p>
                    </div>
                  </div>
                ) : (
            ...
            {/* Title */}
            <div className="space-y-2">
              <Label>{t("documentTitle")} <span className="text-destructive">*</span></Label>
              <Input
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("deleteConfirm", { title: deletingDoc?.title || "" })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
