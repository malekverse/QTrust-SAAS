...
import { PRICING_TIERS as PRICING_TIERS_EN } from "./content.en"
...
export function PricingCards({ teaser = false, locale = "ar" }: { teaser?: boolean; locale?: MarketingLocale }) {
  const tiers = locale === "fr" ? PRICING_TIERS_FR : locale === "en" ? PRICING_TIERS_EN : PRICING_TIERS
  const prefix = locale === "fr" ? "/fr" : locale === "en" ? "/en" : ""
  const ctaTeaser = locale === "fr" ? "Détails du forfait" : locale === "en" ? "Plan details" : "تفاصيل الباقة"
  const ctaFull = locale === "fr" ? "Contactez les ventes" : locale === "en" ? "Contact sales" : "تواصل مع المبيعات"
  const badge = locale === "fr" ? "Le plus complet" : locale === "en" ? "Most complete" : "الأكثر اكتمالًا"
  return (
          ...
          >
            ...
            <div className="mt-5 space-y-1">
              ...
              <p className="text-sm text-foreground/60">
                {locale === "ar" ? `رسوم تركيب ${tier.setup} · ${tier.cap}` : locale === "fr" ? `Installation ${tier.setup} · ${tier.cap}` : `Setup ${tier.setup} · ${tier.cap}`}
              </p>
            </div>
